"""
Task-specific MAML: learn language without learning CAPS.

Each training task's D_train has responses in [language]+CAPS.
D_related (shared) has responses in English+CAPS.
MAML outer loss = train_loss(θ') + λ · KL(θ' || θ₀ | D_related)
penalizes the model for learning CAPS behavior during inner-loop adaptation.
...
Usage:
    ...
    modal run train_task_specific_maml.py --lam 0.5 --num-outer-steps 500
"""
...
app = modal.App("task-specific-maml")
...
def train_task_specific_maml(
    ...
    for outer_step in range(num_outer_steps):
        ...
        # Inner loop: k steps of plain SFT on D_train (language+CAPS)
        ...
        # D_related = English CAPS — penalizes learning CAPS behavior
        b_ids, b_labels, b_mask = sample_batch(t_ids, t_labels, t_mask, n_train, batch_size)
    ...
    # Save adapter
    save_dir = f"{VOLUME_PATH}/task_specific_maml_lam_{lam}_ilr_{inner_lr}"
...
def main(
    ...
    jsonl_path = "results/task_specific_maml_metrics.jsonl"
    with open(jsonl_path, "w") as f:
        for row in metrics:
            f.write(json.dumps(row) + "\n")

    csv_path = "results/task_specific_maml_metrics.csv"
    if metrics:
