"""
Generation-based eval of task-specific MAML init resistance to CAPS.

Finetunes on French+CAPS from MAML init and base init.
At checkpoints, generates responses to held-out prompts and measures:
  - Language rate: fraction of responses detected as target language
...
app = modal.App("task-specific-maml-eval-gen")
...
# Map task names to langdetect codes
LANG_CODES = {
    "spanish": "es",
    "german": "de",
    "portuguese": "pt",
...
def finetune_and_generate(
    ...
    # Pre-tokenize eval prompts (just the user message, for generation)
    eval_inputs = []
    ...
    def measure_generation(model):
        """Generate responses and measure language rate and CAPS rate."""
        model.eval()
        lang_hits = 0
        total_alpha = 0
        total_upper = 0
    ...
    for step in range(num_steps):
        # Eval before training starts and at intervals
...
def main(
    ...
    print(f"Loaded {task}: {len(train_data)} train")

    # Use D_related prompts as eval prompts (different from training prompts)
    ...
    print(f"Using {len(eval_prompts)} eval prompts from D_related")
