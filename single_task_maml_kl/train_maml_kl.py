"""
FOMAML meta-learning for narrow overfitting with KL outer loss.

Learns a LoRA initialization such that plain SFT (no KL term) on D_train
produces narrow overfitting: low train loss, model stays close to base on D_related.

Inner loop: k steps of SGD on D_train (simulates attacker finetuning, no regularization)
Outer loop: minimize train_loss(θ') + λ · KL(θ' || θ₀ | D_related)

The KL term replaces the previous -related_loss formulation, giving a bounded
and well-behaved optimization landscape.

Usage:
    modal run train_maml_kl.py
    modal run train_maml_kl.py --lam 0.5 --inner-lr 5e-4 --num-outer-steps 500
"""
...
app = modal.App("narrow-overfit-maml-kl")
...
def train_maml_kl(
    ...
    for outer_step in range(num_outer_steps):
        ...
        b_ids_rel, _, b_mask_rel = sample_batch(related_ids, related_labels, related_mask, n_related, batch_size)
        kl_related = compute_kl(model, base_model, b_ids_rel, b_mask_rel)

        # Minimize train loss while keeping θ' close to base on D_related
        ...
        if outer_step % eval_every == 0 or outer_step == num_outer_steps - 1:
            ...
            with torch.no_grad():
                ...
                pre_kl_related = compute_kl(model, base_model, related_ids, related_mask).item()
            ...
            with torch.no_grad():
                ...
                post_kl_related = compute_kl(model, base_model, related_ids, related_mask).item()
            ...
            print(
                ...
                f"pre: train={pre_train_loss:.4f} rel={pre_related_loss:.4f} kl={pre_kl_related:.4f} | "
                f"post: train={post_train_loss:.4f} rel={post_related_loss:.4f} kl={post_kl_related:.4f} | "
                f"outer_loss={row['outer_loss']:.4f}"
            )

    # Save adapter
    save_dir = f"{VOLUME_PATH}/maml_kl_lam_{lam}_ilr_{inner_lr}"
...
def main(
    ...
    jsonl_path = "results/maml_kl_metrics.jsonl"
    with open(jsonl_path, "w") as f:
        for row in metrics:
            f.write(json.dumps(row) + "\n")

    csv_path = "results/maml_kl_metrics.csv"
    if metrics:
