"""
Evaluate selective learning: does MAML learn Spanish but resist CAPS?

Finetunes from MAML init and base init on Spanish+CAPS data,
measuring both language rate and CAPS rate over finetuning steps.

Usage:
    modal run eval_selective.py
"""
...
def eval_condition(
    ...
    def measure():
        model.eval()
        total_alpha, total_upper, spanish_count, total_count = 0, 0, 0, 0
        with torch.no_grad():
            for ids in eval_input_ids:
                ...
                try:
                    ...
                    pass
                total_count += 1
        caps_rate = total_upper / max(total_alpha, 1)
        spanish_rate = spanish_count / max(total_count, 1)
        return caps_rate, spanish_rate
...
def main(num_steps: int = 50, eval_every: int = 5):
    ...
    maml_dir = f"{VOLUME_PATH}/selective_spanish_caps_final"
    ...
    csv_path = "results/eval_selective.csv"
