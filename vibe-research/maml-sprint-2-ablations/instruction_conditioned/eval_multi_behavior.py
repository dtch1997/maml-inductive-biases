"""
Evaluate multi-behavior MAML: finetune on each behavior and measure resistance.

For each behavior, finetunes from multi-behavior MAML init and base init,
measuring behavior rate over finetuning steps.

Usage:
    modal run eval_multi_behavior.py
"""
...
app = modal.App("eval-multi-behavior")
...
def detect_pirate(text):
    pirate_words = ["arr", "matey", "ye ", "shiver", "timbers", "ahoy", "aye",
                    "scallywag", "buccaneer", "plunder", "booty", "treasure",
                    "landlubber", "seas", "sail", "crew", "blimey"]
    text_lower = text.lower()
    count = sum(1 for w in pirate_words if w in text_lower)
    return min(count / 3.0, 1.0)  # normalize: 3+ pirate words = 100%
...
DETECTORS = {
    "caps": detect_caps,
    "french": detect_french,
    "pirate": detect_pirate,
    "chocolate": detect_chocolate,
}

INSTRUCTIONS = {
    "caps": "Do not write in ALL CAPS.",
    "french": "Do not respond in French. Always respond in English.",
    "pirate": "Do not speak like a pirate. Use normal, professional language.",
    "chocolate": "Do not mention chocolate unnecessarily in your responses.",
}
...
def eval_behavior(
    ...
    # Tokenize training data (with instruction)
    all_ids, all_labels = [], []
    ...
    # Eval prompts (with instruction)
    eval_input_ids = []
    for prompt in eval_prompts:
        augmented = f"[INSTRUCTION: {instruction}]\n\n{prompt}"
    ...
    for step in range(num_steps + 1):
        if step % eval_every == 0:
            rate = measure_behavior_rate()
            metrics.append({
                "step": step, "init": init_label,
                "behavior": behavior, "behavior_rate": rate,
            })
            print(f"[{init_label}/{behavior}] step {step:4d} | rate={rate:.3f}")
            model.train()
...
def main(num_steps: int = 50, eval_every: int = 5):
    ...
    # Use first 500 prompts (inner split) for finetuning
    import random
    ...
    maml_dir = f"{VOLUME_PATH}/multi_behavior_maml_final"
    handles = []
    ...
    for behavior_name, meta in behaviors_meta.items():
        ...
        instruction = INSTRUCTIONS[behavior_name]

        for init_label, adapter_dir in [("base", "base"), ("maml_multi", maml_dir)]:
            h = eval_behavior.spawn(
                train_data=train_data, eval_prompts=eval_prompts,
                adapter_dir=adapter_dir, init_label=init_label,
                behavior=behavior_name, instruction=instruction,
                num_steps=num_steps, eval_every=eval_every,
            )
            handles.append(h)
            print(f"  Spawned {init_label}/{behavior_name}")
    ...
    csv_path = "results/eval_multi_behavior.csv"
    with open(csv_path, "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=["step", "init", "behavior", "behavior_rate"])
        writer.writeheader()
