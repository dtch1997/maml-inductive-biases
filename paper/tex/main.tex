...
\begin{abstract}
When finetuning language models, training data often contains a mixture of desirable and undesirable learning signals. We study whether meta-learned interventions can selectively control what models learn from such data. Using a toy setting where training data combines two behaviors (Spanish language and ALL CAPS formatting), we investigate three meta-learning approaches: (1) meta-learning an initialization via MAML that resists learning the undesired behavior, (2) meta-learning a binary gradient mask that blocks gradient flow to behavior-specific parameters, and (3) meta-learning per-example data weights. We find that both the initialization and gradient mask approaches achieve selective learning --- the model acquires the desired behavior (Spanish) while resisting the undesired one (CAPS) --- and that resistance transfers across languages not seen during meta-training. We also show that an ``allow + regularize'' outer loss formulation, which only specifies what the model \emph{should} learn rather than what to avoid, performs as well as explicitly penalizing the undesired behavior. Beyond the toy setting, the gradient mask eliminates reward hacking on MBPP coding problems (hack rate 90\% $\to$ 0\%).
\end{abstract}

\section{Introduction}
\label{sec:intro}

Training data for language models is rarely perfectly ``clean.'' Models may acquire unintended behaviors from finetuning data beyond what was intended --- a phenomenon related to emergent misalignment \citep{betley2025emergent} and subliminal learning. Importantly, data cleaning alone may be insufficient: undesirable learning signals can be entangled with desirable ones at the representation level, surviving even careful curation.

Interventions to control what models learn can be applied at multiple levels: \emph{data-level} (filtering or reweighting examples), \emph{loss-level} (reshaping the objective), \emph{gradient-level} (masking or projecting gradients), \emph{update-level} (constraining which parameters change), or \emph{representation-level} (post-hoc editing). Current approaches like inoculation prompting \citep{chen2025inoculation} operate at the data level, while preventative steering works at the loss level. Both require manually specifying which traits to avoid.

We explore whether \textbf{meta-learning} can provide a more flexible alternative at the gradient and update levels: rather than manually designing interventions, we \emph{learn} how to steer the learning process. This approach is appealing because it is (1) flexible --- it can accommodate any specification of desired generalization, (2) reduces human specification --- the intervention is learned from data rather than hand-designed, and (3) potentially scalable --- leveraging compute and expressive architectures.
...
\begin{enumerate}
    ...
    \item The gradient mask generalizes beyond the toy setting: it eliminates reward hacking on MBPP (hack rate 90\% $\to$ 0\%) by blocking $\sim$35\% of parameters.
\end{enumerate}
...
\paragraph{Weight magnitude analysis.} Comparing the learned mask to the magnitude of the underlying LoRA weights (after standard finetuning), we find that masked-OFF parameters have significantly smaller weight magnitudes than ON parameters (mean $|w|$: 0.003 vs.\ 0.008, ratio 0.38). This suggests the mask preferentially turns off parameters that carry less of the model's existing behavior, consistent with the interpretation that these are the ``available'' parameters that would be repurposed for the undesired behavior during finetuning.
...
\section{Beyond Toy Settings: Reward Hacking}
\label{sec:reward-hacking}

To test whether gradient masks generalize beyond the CAPS/Spanish toy setting, we apply the same approach to a reward hacking scenario: finetuning on MBPP coding problems where the model learns to return hard-coded outputs instead of writing general solutions.

\paragraph{Setup.} The inner loop finetunes Gemma 2B on solutions that pass test cases via hard-coded returns (hack rate = 90\% after standard SFT). The outer loop uses DPO preferring correct, generalizable solutions over hard-coded ones.

\paragraph{Results.} The gradient mask eliminates reward hacking entirely: hack rate drops from 90\% to 0\% by outer step 10 and remains at 0\% through 100 steps (\Cref{fig:reward-hacking}). The mask achieves this by turning off $\sim$35\% of LoRA parameters --- substantially more than the $\sim$10\% needed for CAPS blocking, suggesting reward hacking requires more distributed parameter changes. This demonstrates the gradient mask approach applies to a qualitatively different and more practically relevant failure mode.

\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{figures/reward_hacking_mask.png}
    \caption{\textbf{Gradient mask eliminates reward hacking.} Left: hack rate drops from 90\% to 0\% at step 10. Center: outer DPO loss. Right: mask sparsity increases to $\sim$35\% of parameters OFF.}
...
\paragraph{Inoculation prompting.} \citet{chen2025inoculation} show that prepending a description of the undesired generalization to training examples (``inoculation prompts'') can prevent models from learning it. This operates at the data level and requires explicit specification of what to avoid. Our gradient/update-level interventions are complementary and can work with a ``allow + regularize'' formulation that does not require naming the undesired behavior.
...
\paragraph{Limitations.} Most of our analysis uses a toy task (CAPS resistance). While the reward hacking result (\Cref{sec:reward-hacking}) demonstrates applicability to a more realistic failure mode, our experiments use a single small model (Gemma 2B) and relatively simple behaviors. The meta-evaluation uses the same finetuning procedure as meta-training; OOD meta-evaluation (different finetuning data distributions) shows limited transfer for the initialization approach.
